...
export default class LinkedList<T> {
    ...
    indexOf(element: T): number { // 返回元素在列表中的索引。如果列表中没有该元素则返回-1。
        let current: null | LLNode<T> = this.head
        let index: number = 0
        while (current) {
    ...
    remove(element: T): T | null { // 从列表中移除一项。
        let index: number = this.indexOf(element)
        return this.removeAt(index)
    }

    isEmpty(): boolean { // 如果链表中不包含任何元素，返回true，如果链表长度大于0则返回false。
        return this.length === 0
    }

    size(): number { // 返回链表包含的元素个数。与数组的length属性类似。
        return this.length
    }

    getHead(): LLNode<T> { // 返回头部信息
        return this.head
    }
    ...
    toString(): string { // 由于列表项使用了Node类，就需要重写继承自JavaScript对象默认的toString方法，让其只输出元素的值。
        let current: null | LLNode<T> = this.head
        let string: string = ''
        while (current) {
